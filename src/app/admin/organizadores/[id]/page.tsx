    ...
    <div className="p-8">
      ...
      <div className="flex items-start justify-between mb-8 mt-4">
        <div>
          ...
          <p className="text-[#0a0a0a]/25 text-xs mt-0.5">Since {fmtDate(org.created_at)}</p>
        </div>
      ...
      {org.role === "pending_activation" && (
        <div className="mb-6 rounded-2xl p-5 flex items-start gap-4" style={{ background: "rgba(180,83,9,0.06)", border: "1px solid rgba(180,83,9,0.2)" }}>
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold" style={{ color: "#92400e" }}>Account activation request</p>
            <p className="text-xs mt-0.5" style={{ color: "#b45309" }}>The organizer has requested account activation. Use the toggle to approve or reject it.</p>
          </div>
        </div>
      )}
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: "#fff", border: "1px solid rgba(0,0,0,0.07)" }}>
        <div className="px-6 py-4" style={{ borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
          <p className="text-[#0a0a0a] font-semibold text-sm">Events</p>
        </div>
        {(events ?? []).length === 0 ? (
          <p className="text-[#0a0a0a]/25 text-sm p-8 text-center">No events</p>
        ) : (
