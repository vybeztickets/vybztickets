...
const STATUSES = ["Todos", "active", "sold", "cancelled", "expired"];
...
export default function ListingsTable({ listings, stats }: {
  ...
  return (
    <>
      <p className="text-[#0a0a0a]/35 text-sm mb-5">{stats.active} activos · {stats.sold} vendidos · {fmt(stats.totalVolume)} en volumen</p>
      ...
      <div className="flex gap-3 mb-5 flex-wrap">
        <div className="relative flex-1 min-w-52">
          ...
          <input
            type="text"
            placeholder="Buscar evento o vendedor…"
            value={q}
      ...
      <p className="text-[#0a0a0a]/30 text-xs mb-3">{filtered.length} de {listings.length} listings</p>
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <tr key={l.id} style={{ borderBottom: "1px solid rgba(0,0,0,0.04)" }} className="hover:bg-black/[0.01]">
                  ...
                  <td className="px-6 py-4">
                    <p className="text-[#0a0a0a] text-xs">{l.sellerName ?? "Sin nombre"}</p>
