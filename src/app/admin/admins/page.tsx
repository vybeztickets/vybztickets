...
export default function AdminAdminsPage() {
  ...
  return (
    <div className="p-8 max-w-2xl">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[#0a0a0a] mb-1">Admins</h1>
        <p className="text-[#0a0a0a]/40 text-sm">Manage who has access to the admin panel.</p>
      </div>
      ...
      <div className="rounded-2xl overflow-hidden mb-6" style={{ border: "1px solid rgba(0,0,0,0.08)", background: "#fff" }}>
        <div className="px-5 py-4" style={{ borderBottom: "1px solid rgba(0,0,0,0.06)", background: "#f9f9f9" }}>
          <p className="text-sm font-semibold text-[#0a0a0a]">Add admin</p>
          <p className="text-xs text-[#0a0a0a]/40 mt-0.5">The account must already be registered on Vybz.</p>
        </div>
        <div className="p-5">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(0,0,0,0.08)", background: "#fff" }}>
          ...
          {admins.length === 0 ? (
            <p className="px-5 py-8 text-sm text-[#0a0a0a]/25 text-center">No admins</p>
          ) : (
            admins.map((a, i) => (
              <div
                key={a.id}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#0a0a0a]/40">{a.email} · since {fmtDate(a.created_at)}</p>
                  </div>
                </div>
                <button
      ...
      <p className="text-xs text-[#0a0a0a]/25 mt-4">
        Removing an admin reverts their account to the organizer role.
      </p>
    </div>
  );
}
