  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        <p className="text-[#0a0a0a]/40 text-xs">{tickets.length} asistente{tickets.length !== 1 ? "s" : ""}</p>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl py-16 text-center" style={{ border: "1px dashed rgba(0,0,0,0.08)" }}>
          <p className="text-[#0a0a0a]/20 text-sm">Sin asistentes{search ? " para esta búsqueda" : ""}</p>
        </div>
      ) : (
        <>
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <div
                    ...
                    title={t.status === "used" ? "Ingresado" : "No ingresado"}
                  >
        ...
        <>
          ...
          <div className="fixed right-0 top-0 h-full z-50 flex flex-col" style={{ width: 400, background: "#fff", borderLeft: "1px solid rgba(0,0,0,0.08)" }}>
            <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: "1px solid rgba(0,0,0,0.07)" }}>
              <h3 className="text-[#0a0a0a] font-bold text-base">Editar asistente</h3>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-3">
              ...
              <div>
                <label className="block text-[#0a0a0a]/40 text-xs mb-1">Correo electrónico</label>
                <input type="email" value={fEmail} onChange={(e) => setFEmail(e.target.value)} className={inputClass} style={inputStyle} />
              </div>
              <div>
                <label className="block text-[#0a0a0a]/40 text-xs mb-1">Teléfono</label>
                <input type="tel" value={fPhone} onChange={(e) => setFPhone(e.target.value)} placeholder="Opcional" className={inputClass} style={inputStyle} />
              </div>
            </div>
