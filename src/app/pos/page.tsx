  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        if (s.eventId && s.code) { router.replace("/pos/app"); return; }
      } catch {}
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/scan", {
        ...
        body: JSON.stringify({ code: trimmed, type: "pos" }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error ?? "Invalid code"); setLoading(false); return; }
      if (data.type !== "pos") { setError("This code is not for POS access"); setLoading(false); return; }
      localStorage.setItem("vybz_pos_session", JSON.stringify(data));
      router.push("/pos/app");
    } catch {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6" style={{ background: "#0a0a0a" }}>
      <div className="w-full max-w-sm">
        <div className="text-center mb-12">
          ...
          <p className="text-white/30 text-sm mt-1">POS access</p>
        </div>
