...
export default function ScannerPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!parsed.eventId || !parsed.code) throw new Error();
      setSession(parsed);
    } catch {
  ...
  const processFrame = useCallback(() => {
    ...
    if (code && !cooldownRef.current && code.data !== lastCodeRef.current) {
      ...
      fetch("/api/tickets/validate", {
        ...
        body: JSON.stringify({ qrCode: code.data, eventId: session.eventId, scanCode: session.code }),
      })
        .then(r => r.json())
        .then((data: ScanResult) => {
          ...
          setRecentScans(prev => [
            { ...data, time: new Date().toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", second: "2-digit" }) },
            ...prev.slice(0, 19),
          ]);
          setTimeout(() => {
            setResult(null);
  ...
  // Heartbeat — lets organizer see "Connected" status in real time
  useEffect(() => {
    if (!session) return;
    function beat() {
  ...
  const resultBg =
    ...
    result?.status === "already_used" ? "#f59e0b" : "#ef4444";
  ...
  return (
    <div className="fixed inset-0 flex flex-col" style={{ background: "#000", touchAction: "none" }}>
      ...
      <div
        ...
        style={{ background: "radial-gradient(ellipse 70% 60% at 50% 50%, transparent 30%, rgba(0,0,0,0.7) 100%)" }}
      />

      {/* Top bar */}
      <div
        className="relative z-10 flex items-center justify-between px-5"
        style={{ paddingTop: "env(safe-area-inset-top, 16px)", paddingBottom: 12, marginTop: 8 }}
      >
        <div>
          <p className="text-white/40 text-[9px] uppercase tracking-widest">
            {session.label ? session.label : "Scanning at"}
          </p>
          <p className="text-white font-semibold text-sm leading-tight">{session.eventName}</p>
          <p className="text-white/30 text-[10px]">{session.venue}, {session.city}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="text-white text-xs font-semibold">{count} in</span>
          </div>
          <button
      ...
      {!result && !cameraError && (
        <div className="relative z-10 flex justify-center pb-10" style={{ paddingBottom: "env(safe-area-inset-bottom, 40px)" }}>
          <p className="text-white/30 text-sm">Point the camera at the QR code</p>
        </div>
      )}
      ...
      {showRecent && (
        ...
        >
          ...
          >
            <p className="text-white font-semibold">Scan history</p>
          ...
          <div className="flex-1 overflow-y-auto px-5 pb-10">
            {recentScans.length === 0 ? (
              <p className="text-white/30 text-sm text-center mt-10">No scans yet</p>
            ) : (
                ...
                {recentScans.map((s, i) => (
                  ...
                  >
                    <span
                      ...
                      style={{ background: s.status === "valid" ? "#10b981" : s.status === "already_used" ? "#f59e0b" : "#ef4444" }}
                    >
                      {s.status === "valid" ? "✓" : "!"}
                    </span>
      ...
      {showDisconnect && (
        ...
        >
          <div className="w-full max-w-xs rounded-2xl p-6" style={{ background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.1)" }}>
            <p className="text-white font-semibold mb-1">Disconnect</p>
            <p className="text-white/40 text-sm mb-5">You'll need to enter the code again to reconnect.</p>
