...
export function StarButton({
  ...
  const resolvedLightColor = lightColor ?? (dark ? "rgba(255,255,255,0.85)" : "rgba(0,0,0,0.5)");
  const resolvedStarColor = starColor ?? (dark ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.15)");
  ...
  useEffect(() => {
    ...
    // Apply offset-path directly (bypasses CSS variable limitation)
    light.style.setProperty("offset-path", path);
    light.style.animation = `star-btn ${duration}s linear infinite`;
  }, [duration]);

  const sharedClassName = cn(
    "relative overflow-hidden inline-flex items-center justify-center gap-2.5 whitespace-nowrap rounded-full text-sm font-bold px-7 py-3.5 transition-all duration-200",
    dark
  ...
  const sharedStyle: CSSProperties = {
    border: `${borderWidth}px solid ${dark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.1)"}`,
    isolation: "isolate",
  };

  const content = (
    <>
      {/* Orbiting glow — animation applied via JS ref */}
      <div
        ref={lightRef}
        ...
        style={{
          ...
          aspectRatio: "1",
          offsetDistance: "0%",
          background: `radial-gradient(ellipse at center, ${resolvedLightColor} 0%, transparent 60%)`,
          zIndex: 1,
        } as CSSProperties}
      />
      ...
      <div
        ...
        aria-hidden
      >
        <StarBackground color={resolvedStarColor} />
      </div>
      {/* Text */}
      <span className={cn("relative", dark ? "text-white" : "text-[#0a0a0a]")} style={{ zIndex: 3 }}>
        {children}
      </span>
    </>
  );
