...
interface StarButtonBaseProps {
  ...
  borderWidth?: number;
  className?: string;
  /** When true: dark background with white text and light orb */
  dark?: boolean;
}

type StarButtonProps = StarButtonBaseProps &
...
export function StarButton({
  ...
  const resolvedLightColor = lightColor ?? (dark ? "rgba(255,255,255,0.9)" : "#000000");
  const resolvedStarColor = starColor ?? (dark ? "rgba(255,255,255,0.25)" : "rgba(0,0,0,0.18)");
  ...
  const sharedClassName = cn(
    "relative overflow-hidden inline-flex items-center justify-center gap-2.5 whitespace-nowrap rounded-full text-sm font-bold px-7 py-3.5 transition-all duration-200 group/star-button",
    dark
      ? "bg-[#0a0a0a] border border-white/10 hover:bg-[#1a1a1a]"
  ...
  const content = (
    <>
      {/* Orbiting light */}
      <div
        className="absolute aspect-square animate-star-btn pointer-events-none"
        style={
          {
            offsetPath: "var(--path)",
            offsetDistance: "0%",
            width: "var(--light-width)",
            background: `radial-gradient(ellipse at center, ${resolvedLightColor}, transparent, transparent)`,
          } as CSSProperties
        }
      />
      {/* Star dot pattern */}
      <div
        className="absolute inset-0 z-[2] overflow-hidden rounded-[inherit] pointer-events-none"
        style={{ borderWidth: "var(--border-width)", borderColor: "transparent" }}
        aria-hidden
      >
      ...
      <span
        className={cn(
          "z-10 relative inline-block",
          dark
            ? "text-white"
            : "bg-gradient-to-b from-black to-neutral-500 text-transparent bg-clip-text",
        )}
      >
        {children}
      </span>
