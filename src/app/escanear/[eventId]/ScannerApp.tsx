...
export default function ScannerApp({ eventId, eventName }: { eventId: string; eventName: string }) {
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices.getUserMedia({
      ...
      .catch(() => setCameraError("Could not access the camera. Make sure the browser has permission."));
    return () => {
  ...
  return (
    <div className="fixed inset-0 flex flex-col" style={{ background: "#000", touchAction: "none" }}>
      ...
      >
        <div>
          <p className="text-white/40 text-[9px] uppercase tracking-widest">Scanning at</p>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span className="text-white text-xs font-semibold">{count} checked in</span>
          </div>
          <button
      ...
      {!result && !cameraError && (
        <div className="relative z-10 flex justify-center pb-10" style={{ paddingBottom: "env(safe-area-inset-bottom, 40px)" }}>
          <p className="text-white/40 text-sm">Point the camera at the QR code</p>
        </div>
      )}
